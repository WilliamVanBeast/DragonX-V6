...
import dragonclient.event.Events.ActionEvent;
import dragonclient.event.Events.MoveEvent;
...
import dragonclient.util.DiscordWebhookSender;
...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void onUpdateWalkingPlayer() {
		...
		ActionEvent actionEvent = new ActionEvent(this.isSprinting(), this.isSneaking());
		Dragon.eventManager.callEvent(actionEvent);

		final boolean flag = actionEvent.sprinting;
		final boolean flag1 = actionEvent.sneaking;
		...
		if (this.isCurrentViewEntity()) {
			...
			boolean flag2 = d0 * d0 + d1 * d1 + d2 * d2 > (Dragon.moduleManager.getModule(No003.class).isEnabled() ? 0.0 : 9.0E-4D)  || this.positionUpdateTicks >= 20;
			...
			Dragon.eventManager.callEvent(new PostMotionEvent());
	}
	...
	public void onLivingUpdate() {
		Dragon.eventManager.callEvent(new UpdateEvent());
