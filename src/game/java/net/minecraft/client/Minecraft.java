...
public class Minecraft implements IThreadListener {
	...
	private void startGame() throws IOException {
		...
		if (this.serverName != null) {
			this.displayGuiScreen(new GuiConnecting(new GuiScreenEditProfile(new Menu()), this, this.serverName,
					this.serverPort));
		} else {
			this.displayGuiScreen(new GuiScreenEditProfile(new Menu()));
		}
	...
	public void displayGuiScreen(GuiScreen guiScreenIn) {
		...
		if (guiScreenIn instanceof Menu || guiScreenIn instanceof GuiMultiplayer) {
	...
	public void runTick() throws IOException {
		...
		if (this.world != null) {
			if (!this.isGamePaused) {
				...
				try {
					Dragon.eventManager.callEvent(new TickEvent());
					this.world.tick();
		...
		if (reconnectURI != null) {
			...
			if (EagRuntime.getConfiguration().isAllowServerRedirects()) {
				...
				this.displayGuiScreen(
						new GuiConnecting(new Menu(), this, addr.getIP(), addr.getPort(), enableCookies));
			} else {
		...
		while (Keyboard.next()) {
			...
			if (flag) {
				...
				if (this.currentScreen == null) {
					...
					if (Keyboard.isKeyDown(KeyboardConstants.KEY_RSHIFT)) {
						this.displayGuiScreen(Dragon.clickGui);
					}

					if (this.currentScreen != null) {
						this.currentScreen.handleKeyboardInput();
					} else {
						for (dragonclient.module.Module m : Dragon.moduleManager.getModules()) {
							if (m.getKey().get() == i) {
								m.toggle();
							}
						}
	...
	public void launchIntegratedServer(String folderName, String worldName, WorldSettings worldSettingsIn) {
		...
		this.displayGuiScreen(new GuiScreenIntegratedServerBusy(
				new GuiScreenSingleplayerConnecting(new Menu(), "Connecting to " + folderName),
				...
				() -> SingleplayerServerController.isWorldReady(), (t, u) -> {
					Minecraft.this.displayGuiScreen(GuiScreenIntegratedServerBusy.createException(new Menu(),
	...
	public void loadWorld(WorldClient worldClientIn, String loadingMessage) {
		WorldChangedEvent event = new WorldChangedEvent(this.world, worldClientIn);
		Dragon.eventManager.callEvent(event);
