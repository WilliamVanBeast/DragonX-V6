...
public abstract class Arc2D extends RectangularShape {
	...
	public static class Float extends Arc2D implements Serializable {
		...
		/**
		 * Constructs a new OPEN arc, initialized to location (0, 0), size (0, 0),
		 * angular extents (start = 0, extent = 0).
		 * 
		 * @since 1.2
		 */
		public Float() {
		 ...
		 *
		 * @param type The closure type for the arc: {@link #OPEN}, {@link #CHORD}, or
		 *             {@link #PIE}.
		 * @since 1.2
		 */
		 ...
		 * extents, and closure type.
		 *
		 * @param ellipseBounds The framing rectangle that defines the outer boundary of
		 *                      the full ellipse of which this arc is a partial section.
		 * @param start         The starting angle of the arc in degrees.
		 * @param extent        The angular extent of the arc in degrees.
		 * @param type          The closure type for the arc: {@link #OPEN},
		...
		/**
		 * Writes the default serializable fields to the <code>ObjectOutputStream</code>
		 * followed by a byte indicating the arc type of this <code>Arc2D</code>
		 * instance.
		 *
		 * @serialData
		 *             <ol>
		 *             <li>The default serializable fields.
		 *             <li>followed by a <code>byte</code> indicating the arc type
		...
		/**
		 * Reads the default serializable fields from the <code>ObjectInputStream</code>
		 * followed by a byte indicating the arc type of this <code>Arc2D</code>
		 * instance.
		 *
		 * @serialData
	 ...
	 * various accessor methods below.
	 *
	 * @param type The closure type of this arc: {@link #OPEN}, {@link #CHORD}, or
	 *             {@link #PIE}.
	 * @see java.awt.geom.Arc2D.Float
	 * @see java.awt.geom.Arc2D.Double
	 * @since 1.2
	...
	/**
	 * Returns the arc closure type of the arc: {@link #OPEN}, {@link #CHORD}, or
	 * {@link #PIE}.
	 * 
	 ...
	 * of the arc.
	 *
	 * @return A <CODE>Point2D</CODE> object representing the x,y coordinates of the
	...
	/**
	 * Returns the ending point of the arc. This point is the intersection of the
	 * ray from the center defined by the starting angle plus the angular extent of
	 * the arc and the elliptical boundary of the arc.
	 *
	 * @return A <CODE>Point2D</CODE> object representing the x,y coordinates of the
	 ...
	 * @param closure The closure type for the arc: {@link #OPEN}, {@link #CHORD},
	 *                or {@link #PIE}.
	 * @since 1.2
	 */
	 ...
	 * specified values.
	 *
	 * @param loc     The <CODE>Point2D</CODE> representing the coordinates of the
	 *                upper-left corner of the arc.
	 * @param size    The <CODE>Dimension2D</CODE> representing the width and height
	...
	/**
	 * Sets the position, bounds, angular extents, and closure type of this arc to
	 * the specified values. The arc is defined by a center point and a radius
	 * rather than a framing rectangle for the full ellipse.
	 *
	 ...
	 * value. The starting angle of the arc is tangent to the line specified by
	 * points (p1, p2), the ending angle is tangent to the line specified by points
	 * (p2, p3), and the arc has the specified radius.
	 *
	 ...
	 * coordinates. The first set of coordinates is used to determine the angle of
	 * the starting point relative to the arc's center. The second set of
	 * coordinates is used to determine the angle of the end point relative to the
	 * arc's center. The arc will always be non-empty and extend counterclockwise
	 ...
	 * point relative to the arc's center. The arc will always be non-empty and
	 * extend counterclockwise from the first point around to the second point.
	 *
	...
	/**
	 * Sets the closure type of this arc to the specified value: <CODE>OPEN</CODE>,
	 * <CODE>CHORD</CODE>, or <CODE>PIE</CODE>.
	 *
	 ...
	 * between the starting and ending angles and contains the pie wedge, if this
	 * <code>Arc2D</code> has a <code>PIE</code> closure type.
	 * <p>
	 * This method differs from the {@link RectangularShape#getBounds() getBounds}
	 * in that the <code>getBounds</code> method only returns the bounds of the
	 * enclosing ellipse of this <code>Arc2D</code> without considering the starting
	 * and ending angles of this <code>Arc2D</code>.
	 *
	...
	/**
	 * Constructs a <code>Rectangle2D</code> of the appropriate precision to hold
	 * the parameters calculated to be the framing rectangle of this arc.
	 *
	...
	/**
	 * Determines whether or not the specified angle is within the angular extents
	 * of the arc.
	 *
	 * @param angle The angle to test.
	 *
	 * @return <CODE>true</CODE> if the arc contains the angle, <CODE>false</CODE>
	...
	public boolean intersects(double x, double y, double w, double h) {
		...
		/*
		 * Try to catch rectangles that intersect arc in areas outside of rectagle with
		 * left top corner coordinates (min(center x, start point x, end point x),
		 * min(center y, start point y, end point y)) and rigth bottom corner
		 * coordinates (max(center x, start point x, end point x), max(center y, start
		 * point y, end point y)). So we'll check axis segments outside of rectangle
		 * above.
		 */
		...
		/*
		 * For PIE we should check intersection with pie slices; also we should do the
		 * same for arcs with extent is greater than 180, because we should cover case
		 * of rectangle, which situated between center of arc and chord, but does not
		 * intersect the chord.
		 */
	 ...
	 * modifications to the geometry of the arc do not affect any iterations of that
	 ...
	 * <code>Arc2D</code>. The specified <code>Object</code> is equal to this
	 * <code>Arc2D</code> if it is an instance of <code>Arc2D</code> and if its
	 * location, size, arc extents and type are the same as this <code>Arc2D</code>.
	 * 
