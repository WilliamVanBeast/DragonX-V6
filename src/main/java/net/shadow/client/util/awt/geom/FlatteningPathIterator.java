...
public class FlatteningPathIterator implements PathIterator {
	...
	/**
	 * Constructs a new <code>FlatteningPathIterator</code> object that flattens a
	 * path as it iterates over it. The iterator does not subdivide any curve read
	 * from the source iterator to more than 10 levels of subdivision which yields a
	 * maximum of 1024 line segments per curve.
	 * 
	 * @param src      the original unflattened path being iterated over
	 * @param flatness the maximum allowable distance between the control points and
	 *                 the flattened curve
	 */
	...
	/**
	 * Constructs a new <code>FlatteningPathIterator</code> object that flattens a
	 * path as it iterates over it. The <code>limit</code> parameter allows you to
	 * control the maximum number of recursive subdivisions that the iterator can
	 * make before it assumes that the curve is flat enough without measuring
	 * against the <code>flatness</code> parameter. The flattened iteration
	 * therefore never generates more than a maximum of <code>(2^limit)</code> line
	 * segments per curve.
	 * 
	 * @param src      the original unflattened path being iterated over
	 * @param flatness the maximum allowable distance between the control points and
	 *                 the flattened curve
	 * @param limit    the maximum number of recursive subdivisions allowed for any
	 *                 curved segment
	 * @exception <code>IllegalArgumentException</code> if <code>flatness</code> or
	 * <code>limit</code> is less than zero
	 */
	 ...
	 * or SEG_CLOSE. A float array of length 6 must be passed in and can be used to
	 * store the coordinates of the point(s). Each point is stored as a pair of
	 * float x,y coordinates. SEG_MOVETO and SEG_LINETO types return one point, and
	 * SEG_CLOSE does not return any points.
	 * 
	 * @param coords an array that holds the data returned from this method
	 * @return the path segment type of the current path segment.
	 * @exception <code>NoSuchElementException</code> if there are no more elements
	 * in the flattening path to be returned.
	 ...
	 * Returns the coordinates and type of the current path segment in the
	 * iteration. The return value is the path segment type: SEG_MOVETO, SEG_LINETO,
	 * or SEG_CLOSE. A double array of length 6 must be passed in and can be used to
	 * store the coordinates of the point(s). Each point is stored as a pair of
	 * double x,y coordinates. SEG_MOVETO and SEG_LINETO types return one point, and
	 * SEG_CLOSE does not return any points.
	 * 
	 * @param coords an array that holds the data returned from this method
	 * @return the path segment type of the current path segment.
