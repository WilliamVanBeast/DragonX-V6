...
import net.shadow.client.util.awt.Rectangle;
import java.io.Serializable;
import net.shadow.client.util.sun.awt.geom.Curve;
...
public abstract class QuadCurve2D implements Shape, Cloneable {
	...
	public static class Float extends QuadCurve2D implements Serializable {
		...
		/**
		 * Constructs and initializes a <code>QuadCurve2D</code> with coordinates (0, 0,
		 * 0, 0, 0, 0).
		 * 
		 * @since 1.2
		 */
		...
		/**
		 * Constructs and initializes a <code>QuadCurve2D</code> from the specified
		...
		/**
		 * Sets the location of the end points and control point of this curve to the
	 ...
	 * @param offset the index into the array from which to start getting the
	 ...
	 * <code>QuadCurve2D</code> to the specified <code>Point2D</code> coordinates.
	 * 
	 ...
	 * 
	 * @param pts    an array containing <code>Point2D</code> that define coordinate
	 *               values
	 * @param offset the index into <code>pts</code> from which to start getting the
	 *               coordinate values and assigning them to this
	 *               <code>QuadCurve2D</code>
	 * @since 1.2
	 */
	...
	/**
	 * Returns the square of the flatness, or maximum distance of a control point
	 * from the line connecting the end points, of the quadratic curve specified by
	 * the indicated control points.
	 *
	 * @param x1    the X coordinate of the start point
	 * @param y1    the Y coordinate of the start point
	 * @param ctrlx the X coordinate of the control point
	...
	/**
	 * Returns the flatness, or maximum distance of a control point from the line
	 * connecting the end points, of the quadratic curve specified by the indicated
	 * control points.
	 *
	 * @param x1    the X coordinate of the start point
	 * @param y1    the Y coordinate of the start point
	 ...
	 * Returns the square of the flatness, or maximum distance of a control point
	 * from the line connecting the end points, of the quadratic curve specified by
	 * the control points stored in the indicated array at the indicated index.
	 * 
	 * @param coords an array containing coordinate values
	 * @param offset the index into <code>coords</code> from which to to start
	 *               getting the values from the array
	 * @return the flatness of the quadratic curve that is defined by the values in
	...
	/**
	 * Returns the flatness, or maximum distance of a control point from the line
	 * connecting the end points, of the quadratic curve specified by the control
	 * points stored in the indicated array at the indicated index.
	 * 
	 * @param coords an array containing coordinate values
	 * @param offset the index into <code>coords</code> from which to start getting
	 *               the coordinate values
	 * @return the flatness of a quadratic curve defined by the specified array at
	...
	/**
	 * Returns the square of the flatness, or maximum distance of a control point
	 * from the line connecting the end points, of this <code>QuadCurve2D</code>.
	 * 
	 ...
	 * parameters. Either or both of the <code>left</code> and <code>right</code>
	 * objects can be the same as this <code>QuadCurve2D</code> or
	 * <code>null</code>.
	 * 
	 * @param left  the <code>QuadCurve2D</code> object for storing the left or
	 *              first half of the subdivided curve
	 * @param right the <code>QuadCurve2D</code> object for storing the right or
	...
	/**
	 * Subdivides the quadratic curve specified by the <code>src</code> parameter
	 * and stores the resulting two subdivided curves into the <code>left</code> and
	 * <code>right</code> curve parameters. Either or both of the <code>left</code>
	 * and <code>right</code> objects can be the same as the <code>src</code> object
	 * or <code>null</code>.
	 * 
	 ...
	 * curve is the same as the first point in the second subdivided curve. Thus, it
	 * is possible to pass the same array for <code>left</code> and
	 * <code>right</code> and to use offsets such that <code>rightoff</code> equals
	 * <code>leftoff</code> + 4 in order to avoid allocating extra storage for this
	 * common point.
	 * 
	 * @param src      the array holding the coordinates for the source curve
	 * @param srcoff   the offset into the array of the beginning of the the 6
	 *                 source coordinates
	 * @param left     the array for storing the coordinates for the first half of
	 *                 the subdivided curve
	 * @param leftoff  the offset into the array of the beginning of the the 6 left
	 *                 coordinates
	 * @param right    the array for storing the coordinates for the second half of
	 *                 the subdivided curve
	 * @param rightoff the offset into the array of the beginning of the the 6 right
	 *                 coordinates
	 * @since 1.2
	 */
	...
	/**
	 * Solves the quadratic whose coefficients are in the <code>eqn</code> array and
	 * places the non-complex roots back into the same array, returning the number
	 * of roots. The quadratic solved is represented by the equation:
	 * 
	 * <pre>
	 *     eqn = {C, B, A};
	 *     ax^2 + bx + c = 0
	 * </pre>
	 * 
	 * A return value of <code>-1</code> is used to distinguish a constant equation,
	 * which might be always 0 or never 0, from an equation that has no zeroes.
	 * 
	...
	/**
	 * Solves the quadratic whose coefficients are in the <code>eqn</code> array and
	 * places the non-complex roots into the <code>res</code> array, returning the
	 ...
	 * which might be always 0 or never 0, from an equation that has no zeroes.
	 * 
	 * @param eqn the specified array of coefficients to use to solve the quadratic
	 *            equation
	 * @param res the array that contains the non-complex roots resulting from the
	...
	public boolean contains(double x, double y) {
		 ...
		 *
		 * P1 = (x1, y1) - start point of curve P2 = (x2, y2) - end point of curve Pc =
		 * (xc, yc) - control point
		 *
		 * Pq(t) = P1*(1 - t)^2 + 2*Pc*t*(1 - t) + P2*t^2 = = (P1 - 2*Pc + P2)*t^2 +
		 * 2*(Pc - P1)*t + P1 Pl(t) = P1*(1 - t) + P2*t t = [0:1]
		 *
		 ...
		 *
		 * Let's draw a line through P to be parallel to this vector and find the
		 * intersection of the quad curve and the line.
		 *
		 * Pq(t) is point of intersection if system of equations below has the solution.
		 *
		 * L(s) = P + Pq''*s == Pq(t) Pq''*s + (P - Pq(t)) == 0
		 *
		 * | xq''*s + (x - xq(t)) == 0 | yq''*s + (y - yq(t)) == 0
		 *
		 * This system has the solution if rank of its matrix equals to 1. That is,
		 * determinant of the matrix should be zero.
		 *
		 * (y - yq(t))*xq'' == (x - xq(t))*yq''
		 *
		 * Let's solve this equation with 't' variable. Also let kx = x1 - 2*xc + x2 ky
		 * = y1 - 2*yc + y2
		 *
		 ...
		 *
		 * It's easy to check that t0q == t0l. This fact means we can compute t0 only
		 * one time.
		 *
		 * In case t0 < 0 or t0 > 1, we have an intersections outside of shape bounds.
		 * So, P is definitely out of shape.
		 *
		 * In case t0 is inside [0:1], we should calculate Pq(t0) and Pl(t0). We have
		 * three points for now, and all of them lie on one line. So, we just need to
		 * detect, is our point of interest between points of intersections or not.
		 *
		 * If the denominator in the t0q and t0l equations is zero, then the points must
		 * be collinear and so the curve is degenerate and encloses no area. Thus the
		 * result is false.
		 */
	...
	/**
	 * Fill an array with the coefficients of the parametric equation in t, ready
	 * for solving against val with solveQuadratic. We currently have: val = Py(t) =
	 * C1*(1-t)^2 + 2*CP*t*(1-t) + C2*t^2 = C1 - 2*C1*t + C1*t^2 + 2*CP*t - 2*CP*t^2
	 * + C2*t^2 = C1 + (2*CP - 2*C1)*t + (C1 - 2*CP + C2)*t^2 0 = (C1 - val) + (2*CP
	 * - 2*C1)*t + (C1 - 2*CP + C2)*t^2 0 = C + Bt + At^2 C = C1 - val B = 2*CP -
	 * 2*C1 A = C1 - 2*CP + C2
	 */
	...
	/**
	 * Evaluate the t values in the first num slots of the vals[] array and place
	 * the evaluated values back into the same array. Only evaluate t values that
	 * are within the range <0, 1>, including the 0 and 1 ends of the range iff the
	 * include0 or include1 booleans are true. If an "inflection" equation is handed
	 * in, then any points which represent a point of inflection for that quadratic
	...
	/**
	 * Determine where coord lies with respect to the range from low to high. It is
	 * assumed that low <= high. The return value is one of the 5 values BELOW,
	 * LOWEDGE, INSIDE, HIGHEDGE, or ABOVE.
	 */
	...
	/**
	 * Determine if the pttag represents a coordinate that is already in its test
	 * range, or is on the border with either of the two opttags representing
	 * another coordinate that is "towards the inside" of that test range. In other
	 * words, are either of the two "opt" points "drawing the pt inward"?
	 */
	 ...
	 * <code>QuadCurve2D</code>. The iterator for this class is not multi-threaded
	 * safe, which means that this <code>QuadCurve2D</code> class does not guarantee
	 * that modifications to the geometry of this <code>QuadCurve2D</code> object do
	...
	/**
	 * Returns an iteration object that defines the boundary of the flattened shape
	 * of this <code>QuadCurve2D</code>. The iterator for this class is not
	 ...
	 * <code>QuadCurve2D</code> object do not affect any iterations of that geometry
	 * that are already in process.
	 * 
	 * @param at       an optional <code>AffineTransform</code> to apply to the
	 *                 boundary of the shape
	 * @param flatness the maximum distance that the control points for a subdivided
