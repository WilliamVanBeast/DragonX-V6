...
class ArcIterator implements PathIterator {
	...
	/*
	 * btan computes the length (k) of the control segments at the beginning and end
	 * of a cubic bezier that approximates a segment of an arc with extent less than
	 * or equal to 90 degrees. This length (k) will be used to generate the 2 bezier
	 * control points for such a segment.
	 *
	 * Assumptions: a) arc is centered on 0,0 with radius of 1.0 b) arc extent is
	 * less than 90 degrees c) control points should preserve tangent d) control
	 * segments should have equal length
	 *
	 * Initial data: start angle: ang1 end angle: ang2 = ang1 + extent start point:
	 * P1 = (x1, y1) = (cos(ang1), sin(ang1)) end point: P4 = (x4, y4) = (cos(ang2),
	 * sin(ang2))
	 *
	 * Control points: P2 = (x2, y2) | x2 = x1 - k * sin(ang1) = cos(ang1) - k *
	 * sin(ang1) | y2 = y1 + k * cos(ang1) = sin(ang1) + k * cos(ang1)
	 *
	 * P3 = (x3, y3) | x3 = x4 + k * sin(ang2) = cos(ang2) + k * sin(ang2) | y3 = y4
	 * - k * cos(ang2) = sin(ang2) - k * cos(ang2)
	 *
	 * The formula for this length (k) can be found using the following derivations:
	 *
	 * Midpoints: a) bezier (t = 1/2) bPm = P1 * (1-t)^3 + 3 * P2 * t * (1-t)^2 + 3
	 * * P3 * t^2 * (1-t) + P4 * t^3 = = (P1 + 3P2 + 3P3 + P4)/8
	 *
	 ...
	 *
	 * Since this formula can generate "NaN" values for small angles, we will derive
	 * a safer form that does not involve dividing by very small values: (1 -
	 * cos(angb)) / sin(angb) = = (1 - cos(angb))*(1 + cos(angb)) / sin(angb)*(1 +
	 * cos(angb)) = = (1 - cos(angb)^2) / sin(angb)*(1 + cos(angb)) = = sin(angb)^2
	 * / sin(angb)*(1 + cos(angb)) = = sin(angb) / (1 + cos(angb))
	 *
	 */
	 ...
	 * SEG_QUADTO, SEG_CUBICTO, or SEG_CLOSE. A float array of length 6 must be
	 * passed in and may be used to store the coordinates of the point(s). Each
	 * point is stored as a pair of float x,y coordinates. SEG_MOVETO and SEG_LINETO
	 * types will return one point, SEG_QUADTO will return two points, SEG_CUBICTO
	 * will return 3 points and SEG_CLOSE will not return any points.
	 * 
	 * @see #SEG_MOVETO
	 * @see #SEG_LINETO
	 * @see #SEG_QUADTO
	 ...
	 * Returns the coordinates and type of the current path segment in the
	 * iteration. The return value is the path segment type: SEG_MOVETO, SEG_LINETO,
	 * SEG_QUADTO, SEG_CUBICTO, or SEG_CLOSE. A double array of length 6 must be
	 * passed in and may be used to store the coordinates of the point(s). Each
	 * point is stored as a pair of double x,y coordinates. SEG_MOVETO and
	 * SEG_LINETO types will return one point, SEG_QUADTO will return two points,
