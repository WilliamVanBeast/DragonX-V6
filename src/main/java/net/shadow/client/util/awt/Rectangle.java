...
public class Rectangle extends Rectangle2D implements Shape, java.io.Serializable {
	 ...
	 * 
	 * @param r the <code>Rectangle</code> from which to copy initial values to a
	...
	/**
	 * Constructs a new <code>Rectangle</code> whose upper-left corner is specified
	 * as {@code (x,y)} and whose width and height are specified by the arguments of
	 * the same name.
	 * 
	...
	/**
	 * Constructs a new <code>Rectangle</code> whose upper-left corner is at
	 * (0,&nbsp;0) in the coordinate space, and whose width and height are specified
	...
	/**
	 * Constructs a new <code>Rectangle</code> whose upper-left corner is specified
	 * by the {@link Point} argument, and whose width and height are specified by
	...
	/**
	 * Constructs a new <code>Rectangle</code> whose top left corner is (0,&nbsp;0)
	 * and whose width and height are specified by the <code>Dimension</code>
	 * argument.
	 * 
	...
	/**
	 * Sets the bounding <code>Rectangle</code> of this <code>Rectangle</code> to
	 * match the specified <code>Rectangle</code>.
	 * <p>
	 * This method is included for completeness, to parallel the
	...
	/**
	 * Sets the bounding <code>Rectangle</code> of this <code>Rectangle</code> to
	 * the specified <code>x</code>, <code>y</code>, <code>width</code>, and
	 * <code>height</code>.
	 * <p>
	 * This method is included for completeness, to parallel the
	 * <code>setBounds</code> method of <code>Component</code>.
	 ...
	 * {@code height}. If the parameters specify a {@code Rectangle} that exceeds
	 * the maximum range of integers, the result will be the best representation of
	 * the specified {@code Rectangle} intersected with the maximum integer bounds.
	 * 
	...
	/**
	 * Translates this <code>Rectangle</code> the indicated distance, to the right
	...
	/**
	 * Checks whether or not this <code>Rectangle</code> contains the point at the
	 ...
	 *
	 * @param r the specified <code>Rectangle</code>
	 * @return <code>true</code> if the <code>Rectangle</code> is contained entirely
	 ...
	 * <code>Rectangle</code>. Returns a new <code>Rectangle</code> that represents
	 * the intersection of the two rectangles. If the two rectangles do not
	 * intersect, the result will be an empty rectangle.
	 *
	 * @param r the specified <code>Rectangle</code>
	 * @return the largest <code>Rectangle</code> contained in both the specified
	 *         <code>Rectangle</code> and in this <code>Rectangle</code>; or if the
	 *         rectangles do not intersect, an empty rectangle.
	 */
	 ...
	 * <p>
	 * If either {@code Rectangle} has any dimension less than zero the rules for
	 * <a href=#NonExistant>non-existant</a> rectangles apply. If only one has a
	 * dimension less than zero, then the result will be a copy of the other
	 * {@code Rectangle}. If both have dimension less than zero, then the result
	 ...
	 * <a href=#NonExistant>non-existant</a> rectangles apply. In that case, the new
	 * bounds of this {@code Rectangle} will have a location equal to the specified
	 * coordinates and width and height equal to zero.
	 * <p>
	 * After adding a point, a call to <code>contains</code> with the added point as
	 * an argument does not necessarily return <code>true</code>. The
	 * <code>contains</code> method does not return <code>true</code> for points on
	 * the right or bottom edges of a <code>Rectangle</code>. Therefore, if the
	 * added point falls on the right or bottom edge of the enlarged
	 * <code>Rectangle</code>, <code>contains</code> returns <code>false</code> for
	 * that point. If the specified point must be contained within the new
	 ...
	 * If this {@code Rectangle} has any dimension less than zero, the rules for
	 * <a href=#NonExistant>non-existant</a> rectangles apply. In that case, the new
	 * bounds of this {@code Rectangle} will have a location equal to the
	 * coordinates of the specified {@code Point} and width and height equal to
	 * zero.
	 * <p>
	 * After adding a <code>Point</code>, a call to <code>contains</code> with the
	 * added <code>Point</code> as an argument does not necessarily return
	 * <code>true</code>. The <code>contains</code> method does not return
	 * <code>true</code> for points on the right or bottom edges of a
	 * <code>Rectangle</code>. Therefore if the added <code>Point</code> falls on
	 * the right or bottom edge of the enlarged <code>Rectangle</code>,
	 * <code>contains</code> returns <code>false</code> for that <code>Point</code>.
	 * If the specified point must be contained within the new {@code Rectangle}, a
	 * 1x1 rectangle should be added instead:
	 * 
	 * <pre>
	 * r.add(pt.x, pt.y, 1, 1);
	 * </pre>
	...
	/**
	 * Adds a <code>Rectangle</code> to this <code>Rectangle</code>. The resulting
	 * <code>Rectangle</code> is the union of the two rectangles.
	 * <p>
	 * If either {@code Rectangle} has any dimension less than 0, the result will
	 * have the dimensions of the other {@code Rectangle}. If both
	 * {@code Rectangle}s have at least one dimension less than 0, the result will
	 * have at least one dimension less than 0.
	 * <p>
	 * If either {@code Rectangle} has one or both dimensions equal to 0, the result
	 * along those axes with 0 dimensions will be equivalent to the results obtained
	 * by adding the corresponding origin coordinate to the result rectangle along
	 * that axis, similar to the operation of the {@link #add(Point)} method, but
	 * contribute no further dimension beyond that.
	 * <p>
	 * If the resulting {@code Rectangle} would have a dimension too large to be
	 * expressed as an {@code int}, the result will have a dimension of
	 * {@code Integer.MAX_VALUE} along that dimension.
	 ...
	 * <p>
	 * This method modifies the <code>Rectangle</code> so that it is <code>h</code>
	 * units larger on both the left and right side, and <code>v</code> units larger
	 * at both the top and bottom.
	 * <p>
	 * The new <code>Rectangle</code> has {@code (x - h, y - v)} as its upper-left
	 * corner, width of {@code (width + 2h)}, and a height of {@code (height + 2v)}.
	 * <p>
	 * If negative values are supplied for <code>h</code> and <code>v</code>, the
	 * size of the <code>Rectangle</code> decreases accordingly. The {@code grow}
	 * method will check for integer overflow and underflow, but does not check
	 * whether the resulting values of {@code width} and {@code height} grow from
	...
	public int outcode(double x, double y) {
		/*
		 * Note on casts to double below. If the arithmetic of x+w or y+h is done in
		 * int, then we may get integer overflow. By converting to double before the
		 * addition we force the addition to be carried out in double to avoid overflow
		 * in the comparison.
		 *
	 ...
	 * <code>null</code> and is a <code>Rectangle</code> object that has the same
	 * upper-left corner, width, and height as this <code>Rectangle</code>.
	 * 
	...
	/**
	 * Returns a <code>String</code> representing this <code>Rectangle</code> and
	 * its values.
	 * 
