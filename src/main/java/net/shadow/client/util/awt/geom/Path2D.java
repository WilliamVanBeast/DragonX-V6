...
public abstract class Path2D implements Shape, Cloneable {
	...
	/**
	 * Constructs a new empty {@code Path2D} object. It is assumed that the package
	 * sibling subclass that is defaulting to this constructor will fill in all
	 * values.
	 *
	 * @since 1.6
	 */
	 ...
	 * values. This method is only intended for internal use and should not be made
	 * public if the other constructors for this class are ever exposed.
	 *
	...
	public static class Float extends Path2D implements Serializable {
		 ...
		 * {@code (x1,y1)} as a quadratic parametric control point. All coordinates are
		 ...
		 *             <li>followed by an integer indicating the number of path segments
		 *             to follow (NP) or -1 to indicate an unknown number of path
		 *             segments follows
		 *             <li>followed by an integer indicating the total number of
		 *             coordinates to follow (NC) or -1 to indicate an unknown number of
		 *             coordinates follows (NC should always be even since coordinates
		 *             always appear in pairs representing an x,y pair)
	...
	/**
	 * The {@code Double} class defines a geometric path with coordinates stored in
	...
	public static class Double extends Path2D implements Serializable {
		...
		/**
		 * Constructs a new empty double precision {@code Path2D} object with a default
		 * winding rule of {@link #WIND_NON_ZERO}.
		 *
		 * @since 1.6
		 */
		 ...
		 * Constructs a new empty double precision {@code Path2D} object with the
		 * specified winding rule and the specified initial capacity to store path
		 * segments. This number is an initial guess as to how many path segments are in
		 * the path, but the storage is expanded as needed to store whatever path
		 ...
		 * @return a new {@code PathIterator} that iterates along the boundary of this
		 *         {@code Shape} and provides access to the geometry of this
	 ...
	 * a line segment. If the {@code connect} parameter is {@code true} and the path
	 * is not empty then any initial {@code moveTo} in the geometry of the appended
	 * {@code Shape} is turned into a {@code lineTo} segment. If the destination
	 * coordinates of such a connecting {@code lineTo} segment match the ending
	 * coordinates of a currently open subpath then the segment is omitted as
	 ...
	 * <p>
	 * This method provides a basic facility for implementors of the {@link Shape}
	 * interface to implement support for the {@link Shape#intersects(Rectangle2D)}
	 * method.
	 * <p>
	 * This method object may conservatively return true in cases where the
	 * specified rectangular area intersects a segment of the path, but that segment
