...
package net.shadow.client.util.awt.geom;

import net.shadow.client.util.awt.Shape;
...
public class AffineTransform implements Cloneable, java.io.Serializable {
	...
	/*
	 * This constant is only useful for the cached type field. It indicates that the
	 * type has been decached and must be recalculated.
	 */
	 ...
	 * is anything other than the identity transform, the type will either be the
	 ...
	 * translation in addition to the conversions indicated by other flag bits. A
	...
	/**
	 * This flag bit indicates that the transform defined by this object performs a
	 * uniform scale in addition to the conversions indicated by other flag bits. A
	 * uniform scale multiplies the length of vectors by the same amount in both the
	 * x and y directions without changing the angle between vectors. This flag bit
	...
	/**
	 * This flag bit indicates that the transform defined by this object performs a
	 * general scale in addition to the conversions indicated by other flag bits. A
	 * general scale multiplies the length of vectors by different amounts in the x
	 * and y directions without changing the angle between perpendicular vectors.
	...
	/**
	 * This flag bit indicates that the transform defined by this object performs a
	 * mirror image flip about some axis which changes the normally right handed
	 * coordinate system into a left handed system in addition to the conversions
	 * indicated by other flag bits. A right handed coordinate system is one where
	 * the positive X axis rotates counterclockwise to overlay the positive Y axis
	 * similar to the direction that the fingers on your right hand curl when you
	 * stare end on at your thumb. A left handed coordinate system is one where the
	 * positive X axis rotates clockwise to overlay the positive Y axis similar to
	 * the direction that the fingers on your left hand curl. There is no
	 * mathematical way to determine the angle of the original flipping or mirroring
	 * transformation since all angles of flip are identical given an appropriate
	 * adjusting rotation.
	 * 
	 * @see #TYPE_IDENTITY
	 * @see #TYPE_TRANSLATION
	...
	/*
	 * NOTE: TYPE_FLIP was added after GENERAL_TRANSFORM was in public circulation
	 * and the flag bits could no longer be conveniently renumbered without
	 * introducing binary incompatibility in outside code.
	 */

	/**
	 * This flag bit indicates that the transform defined by this object performs a
	 * quadrant rotation by some multiple of 90 degrees in addition to the
	 * conversions indicated by other flag bits. A rotation changes the angles of
	 * vectors by the same amount regardless of the original direction of the vector
	...
	/**
	 * This flag bit indicates that the transform defined by this object performs a
	 * rotation by an arbitrary angle in addition to the conversions indicated by
	 * other flag bits. A rotation changes the angles of vectors by the same amount
	 * regardless of the original direction of the vector and without changing the
	 * length of the vector. This flag bit is mutually exclusive with the
	 ...
	 * arbitrary conversion of the input coordinates. If this transform can be
	 * classified by any of the above constants, the type will either be the
	 * constant TYPE_IDENTITY or a combination of the appropriate flag bits for the
	 ...
	 * to be copied to their destinations to complete the transformation equation of
	 * this transform.
	 * 
	 * @see #APPLY_TRANSLATE
	 * @see #APPLY_SCALE
	 ...
	 * complete the transformation equation of this transform. The presence of this
	 * bit in the state variable changes the interpretation of the APPLY_SCALE bit
	...
	/*
	 * For methods which combine together the state of two separate transforms and
	 * dispatch based upon the combination, these constants specify how far to shift
	 * one of the states so that the two states are mutually non-interfering and
	 * provide constants for testing the bits of the shifted (HI) state. The methods
	 * in this class use the convention that the state of "this" transform is
	 * unshifted and the state of the "other" or "argument" transform is shifted
	 * (HI).
	 */
	...
	/**
	 * The X coordinate of the translation element of the 3x3 affine transformation
	 * matrix.
	 *
	 * @serial
	 */
	...
	/**
	 * The Y coordinate of the translation element of the 3x3 affine transformation
	 * matrix.
	 *
	 * @serial
	 */
	 ...
	 * values representing either the 4 non-translation enries or the 6 specifiable
	 ...
	 *                   array is less than 6, only the first 4 values are taken. If
	 *                   the length of the array is greater than 6, the first 6
	 *                   values are taken.
	 * @since 1.2
	 */
	 ...
	 * precision values representing either the 4 non-translation entries or the 6
	 ...
	 *           direction
	 * @return an <code>AffineTransform</code> object that represents a translation
	 *         transformation, created with the specified vector.
	 * @since 1.2
	 */
	 ...
	 * and finally translating so that the intermediate origin is restored to the
	 ...
	 * All coordinates rotate about the origin by the same amount. The amount of
	 * rotation is such that coordinates along the former positive X axis will
	 * subsequently align with the vector pointing from the origin to the specified
	 * vector coordinates. If both <code>vecx</code> and <code>vecy</code> are 0.0,
	 * an identity transform is returned. This operation is equivalent to calling:
	 * 
	 * <pre>
	 ...
	 * accordinate to a rotation vector. All coordinates rotate about the specified
	 * anchor coordinates by the same amount. The amount of rotation is such that
	 * coordinates along the former positive X axis will subsequently align with the
	 * vector pointing from the origin to the specified vector coordinates. If both
	 * <code>vecx</code> and <code>vecy</code> are 0.0, an identity transform is
	 ...
	 * </pre>
	 * 
	 * Rotating by a positive number of quadrants rotates points on the positive X
	 * axis toward the positive Y axis.
	 * 
	 * @param numquadrants the number of 90 degree arcs to rotate by
	 * @return an <code>AffineTransform</code> object that rotates coordinates by
	...
	/**
	 * Returns a transform that rotates coordinates by the specified number of
	 * quadrants around the specified anchor point. This operation is equivalent to
	 * calling:
	 * 
	 * <pre>
	 ...
	 * @param anchory      the Y coordinate of the rotation anchor point
	 * @return an <code>AffineTransform</code> object that rotates coordinates by
	 *         the specified number of quadrants around the specified anchor point.
	 * @since 1.6
	 */
	 ...
	 *           direction
	 * @return an <code>AffineTransform</code> object that scales coordinates by the
	 *         specified factors.
	 * @since 1.2
	 */
	 ...
	 * @return an <code>AffineTransform</code> object that shears coordinates by the
	 ...
	 * transform. The return value is either one of the constants TYPE_IDENTITY or
	 * TYPE_GENERAL_TRANSFORM, or a combination of the appriopriate flag bits. A
	 * valid combination of flag bits is an exclusive OR operation that can combine
	 * the TYPE_TRANSLATION flag bit in addition to either of the TYPE_UNIFORM_SCALE
	 * or TYPE_GENERAL_SCALE flag bits as well as either of the
	 * TYPE_QUADRANT_ROTATION or TYPE_GENERAL_ROTATION flag bits.
	 * 
	 * @return the OR combination of any of the indicated flags that apply to this
	 *         transform
	 * @see #TYPE_IDENTITY
	 * @see #TYPE_TRANSLATION
	 * @see #TYPE_UNIFORM_SCALE
	...
	/**
	 * This is the utility function to calculate the flag bits when they have not
	 * been cached.
	 * 
	 * @see #getType
	 */
	 ...
	 * determinant is useful both to determine if the transform can be inverted and
	 ...
	 * various methods that depend on the inverse transform do not need to throw a
	 * {@link NoninvertibleTransformException}. If the determinant is zero then this
	 * transform can not be inverted since the transform maps all input coordinates
	 * onto a line or a point. If the determinant is near enough to zero then
	 * inverse transform operations might not carry enough precision to produce
	 * meaningful results.
	 * <p>
	 * If this transform represents a uniform scale, as indicated by the
	 * <code>getType</code> method then the determinant also represents the square
	 * of the uniform scale factor by which all of the points are expanded from or
	 * contracted towards the origin. If this transform represents a non-uniform
	 * scale or more general transform then the determinant is not likely to
	 * represent a value useful for any purpose other than determining if inverse
	...
	/**
	 * Manually recalculates the state of the transform when the matrix changes too
	 * much to predict the effects on the state. The following table specifies what
	 * the various settings of the state field say about the values of the
	 * corresponding matrix element fields. Note that the rules governing the SCALE
	 * fields are slightly different depending on whether the SHEAR flag is also
	 * set.
	 * 
	 * <pre>
	 ...
	 * and places them into an array of double precisions values. The values are
	 * stored in the array as
	 * {&nbsp;m00&nbsp;m10&nbsp;m01&nbsp;m11&nbsp;m02&nbsp;m12&nbsp;}. An array of 4
	 * doubles can also be specified, in which case only the first four elements
	 * representing the non-transform parts of the array are retrieved and the
	 * values are stored into the array as
	 * {&nbsp;m00&nbsp;m10&nbsp;m01&nbsp;m11&nbsp;}
	 * 
	 ...
	 * 
	 * @return a double value that is the X coordinate of the scaling element of the
	 ...
	 * transformation matrix.
	 * 
	 * @return a double value that is the Y coordinate of the scaling element of the
	 *         affine transformation matrix.
	 * @see #getMatrix
	 * @since 1.2
	 */
	 ...
	 * transformation matrix.
	 * 
	 * @return a double value that is the X coordinate of the translation element of
	 *         the affine transformation matrix.
	 * @see #getMatrix
	 * @since 1.2
	 */
	 ...
	 * transformation matrix.
	 * 
	 * @return a double value that is the Y coordinate of the translation element of
	 *         the affine transformation matrix.
	 * @see #getMatrix
	 * @since 1.2
	 */
	...
	private static final int rot90conversion[] = { /* IDENTITY => */ APPLY_SHEAR,
			/* TRANSLATE (TR) => */ APPLY_SHEAR | APPLY_TRANSLATE, /* SCALE (SC) => */ APPLY_SHEAR,
			/* SC | TR => */ APPLY_SHEAR | APPLY_TRANSLATE, /* SHEAR (SH) => */ APPLY_SCALE,
			/* SH | TR => */ APPLY_SCALE | APPLY_TRANSLATE, /* SH | SC => */ APPLY_SHEAR | APPLY_SCALE,
	 ...
	 * an anchor point. This operation is equivalent to translating the coordinates
	 * so that the anchor point is at the origin (S1), then rotating them about the
	 * new origin (S2), and finally translating so that the intermediate origin is
	 ...
	 * according to a rotation vector. All coordinates rotate about the origin by
	 * the same amount. The amount of rotation is such that coordinates along the
	 * former positive X axis will subsequently align with the vector pointing from
	 * the origin to the specified vector coordinates. If both <code>vecx</code> and
	 * <code>vecy</code> are 0.0, no additional rotation is added to this transform.
	...
	/**
	 * Concatenates this transform with a transform that rotates coordinates around
	 * an anchor point according to a rotation vector. All coordinates rotate about
	 * the specified anchor coordinates by the same amount. The amount of rotation
	 * is such that coordinates along the former positive X axis will subsequently
	 * align with the vector pointing from the origin to the specified vector
	 * coordinates. If both <code>vecx</code> and <code>vecy</code> are 0.0, the
	 * transform is not modified in any way. This method is equivalent to calling:
	 * 
	 * <pre>
	...
	/**
	 * Concatenates this transform with a transform that rotates coordinates by the
	 * specified number of quadrants. This is equivalent to calling:
	 * 
	 * <pre>
	...
	/**
	 * Concatenates this transform with a transform that rotates coordinates by the
	 * specified number of quadrants around the specified anchor point. This method
	 ...
	 * to calling concatenate(S), where S is an <code>AffineTransform</code>
	...
	/**
	 * Sets this transform to a translated rotation transformation. This operation
	 * is equivalent to translating the coordinates so that the anchor point is at
	 * the origin (S1), then rotating them about the new origin (S2), and finally
	 * translating so that the intermediate origin is restored to the coordinates of
	...
	/**
	 * Sets this transform to a rotation transformation that rotates coordinates
	 * around an anchor point according to a rotation vector. All coordinates rotate
	 * about the specified anchor coordinates by the same amount. The amount of
	 * rotation is such that coordinates along the former positive X axis will
	 * subsequently align with the vector pointing from the origin to the specified
	 * vector coordinates. If both <code>vecx</code> and <code>vecy</code> are 0.0,
	 * the transform is set to an identity transform. This operation is equivalent
	 * to calling:
	 * 
	 * <pre>
	...
	/**
	 * Sets this transform to a rotation transformation that rotates coordinates by
	 * the specified number of quadrants. This operation is equivalent to calling:
	 * 
	 * <pre>
	 ...
	 * coordinates by the specified number of quadrants around the specified anchor
	 ...
	 * <code>AffineTransform</code> object.
	 * 
	 * @param Tx the <code>AffineTransform</code> object from which to copy the
	 *           transform
	 * @since 1.2
	 */
	 ...
	 * <code>AffineTransform</code> Cx in the most commonly useful way to provide a
	 * new user space that is mapped to the former user space by <code>Tx</code>. Cx
	 * is updated to perform the combined transformation. Transforming a point p by
	 * the updated transform Cx' is equivalent to first transforming p by
	 * <code>Tx</code> and then transforming the result by the original transform Cx
	 * like this: Cx'(p) = Cx(Tx(p)) In matrix notation, if this transform Cx is
	 ...
	 * updated transform Cx' is equivalent to first transforming p by the original
	 * transform Cx and then transforming the result by <code>Tx</code> like this:
	 * Cx'(p) = Tx(Cx(p)) In matrix notation, if this transform Cx is represented by
	 * the matrix [this] and <code>Tx</code> is represented by the matrix [Tx] then
	 ...
	 * <p>
	 * If this transform maps all coordinates onto a point or a line then it will
	 * not have an inverse, since coordinates that do not lie on the destination
	 * point or line will not have an inverse mapping. The
	 * <code>getDeterminant</code> method can be used to determine if this transform
	 * has no inverse, in which case an exception will be thrown if the
	 * <code>createInverse</code> method is called.
	 * 
	 * @return a new <code>AffineTransform</code> object representing the inverse
	 *         transformation.
	 * @see #getDeterminant
	...
	/**
	 * Sets this transform to the inverse of itself. The inverse transform Tx' of
	 * this transform Tx maps coordinates transformed by Tx back to their original
	 ...
	 * <code>getDeterminant</code> method can be used to determine if this transform
	 * has no inverse, in which case an exception will be thrown if the
	 * <code>invert</code> method is called.
	 * 
	 * @see #getDeterminant
	 * @exception NoninvertibleTransformException if the matrix cannot be inverted.
	 * @since 1.6
	 ...
	 * {@link Point2D} object is allocated and then the result of the transformation
	 * is stored in this object. In either case, <code>ptDst</code>, which contains
	 * the transformed point, is returned for convenience. If <code>ptSrc</code> and
	 ...
	 * <code>ptDst</code> array is <code>null</code>, a new <code>Point2D</code>
	 * object is allocated and stored into that element before storing the results
	 * of the transformation.
	 * <p>
	 * Note that this method does not take any precautions to avoid problems caused
	 * by storing results into <code>Point2D</code> objects that will be used as the
	 * source for calculations further down the source array. This method does
	 * guarantee that if a specified <code>Point2D</code> object is both the source
	 * and destination for the same single point transform operation then the
	 * results will not be stored until the calculations are complete to avoid
	 * storing the results on top of the operands. If, however, the destination
	 * <code>Point2D</code> object for one operation is the same object as the
	 * source <code>Point2D</code> object for another operation further down the
	 * source array then the original coordinates in that point are overwritten
	 ...
	 * This method ensures that no source coordinates are overwritten by a previous
	 * operation before they can be transformed. The coordinates are stored in the
	 * arrays starting at the specified offset in the order
	 ...
	 * stores the results into an array of doubles. The coordinates are stored in
	 ...
	 * stores the results into an array of floats. The coordinates are stored in the
	 * arrays starting at the specified offset in the order
	 * <code>[x0, y0, x1, y1, ..., xn, yn]</code>.
	 * 
	 * @param srcPts the array containing the source point coordinates. Each point
	...
	/**
	 * Inverse transforms the specified <code>ptSrc</code> and stores the result in
	 * <code>ptDst</code>. If <code>ptDst</code> is <code>null</code>, a new
	 * <code>Point2D</code> object is allocated and then the result of the transform
	 * is stored in this object. In either case, <code>ptDst</code>, which contains
	 * the transformed point, is returned for convenience. If <code>ptSrc</code> and
	 * <code>ptDst</code> are the same object, the input point is correctly
	 * overwritten with the transformed point.
	 ...
	 * transform. The two coordinate array sections can be exactly the same or can
	 * be overlapping sections of the same array without affecting the validity of
	...
	/**
	 * Transforms the relative distance vector specified by <code>ptSrc</code> and
	 * stores the result in <code>ptDst</code>. A relative distance vector is
	 * transformed without applying the translation components of the affine
	 ...
	 * </pre>
	 * 
	 * If <code>ptDst</code> is <code>null</code>, a new <code>Point2D</code> object
	 * is allocated and then the result of the transform is stored in this object.
	 * In either case, <code>ptDst</code>, which contains the transformed point, is
	 * returned for convenience. If <code>ptSrc</code> and <code>ptDst</code> are
	 * the same object, the input point is correctly overwritten with the
	 * transformed point.
	 * 
	 ...
	 * results. This method ensures that no source coordinates are overwritten by a
	 ...
	 *               returned. Each vector is stored as a pair of relative x,&nbsp;y
	 *               coordinates.
	 * @param srcOff the offset to the first vector to be transformed in the source
	 *               array
	 * @param dstOff the offset to the location of the first transformed vector that
	 *               is stored in the destination array
	...
	/**
	 * Returns a new {@link Shape} object defined by the geometry of the specified
	 * <code>Shape</code> after it has been transformed by this transform.
	 * 
	 * @param pSrc the specified <code>Shape</code> object to be transformed by this
	 *             transform.
	 ...
	 * transform.
	 * 
	 * @return <code>true</code> if this <code>AffineTransform</code> is an identity
	 ...
	 * same affine coordinate transform as the specified argument.
	 * 
	...
	/*
	 * Serialization support. A readObject method is neccessary because the state
	 * field is part of the implementation of this particular AffineTransform and
	 * not part of the public specification. The state variable's value needs to be
	 * recalculated on the fly by the readObject method as it is in the 6-argument
	 * matrix constructor.
	 */
