package net.shadow.client.util.math;
...
public class ClientMathUtils {
	...
	public static float normalizeValue(float p_148266_1_, float valueMin, float valueMax, float valueStep) {
		return MathHelper.clamp(
				(snapToStepClamp(p_148266_1_, valueMin, valueMax, valueStep) - valueMin) / (valueMax - valueMin), 0.0F,
				1.0F);
	}
	...
	public static float denormalizeValue(float value, float valueMin, float valueMax, float valueStep) {
		return snapToStepClamp(valueMin + (valueMax - valueMin) * MathHelper.clamp(value, 0.0F, 1.0F), valueMin,
				valueMax, valueStep);
	}
