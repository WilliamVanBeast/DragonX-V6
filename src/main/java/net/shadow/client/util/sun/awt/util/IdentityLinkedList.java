...
package net.shadow.client.util.sun.awt.util;
 ...
 * <tt>IdentityLinkedList</tt> class provides uniformly named methods to
 ...
 * stack, {@linkplain Queue queue}, or {@linkplain Deque double-ended queue}.
 * <p>
 *
...
public class IdentityLinkedList<E> extends AbstractSequentialList<E> implements List<E>, Deque<E> {
	 ...
	 * <tt>get(i)==o</tt> (if such an element exists). Returns <tt>true</tt> if this
	 * list contained the specified element (or equivalently, if this list changed
	 ...
	 * the lowest index <tt>i</tt> such that <tt>get(i)==o</tt>, or -1 if there is
	 * no such index.
	 *
	 ...
	 * list, or -1 if this list does not contain the element. More formally, returns
	 * the highest index <tt>i</tt> such that <tt>get(i)==o</tt>, or -1 if there is
	 * no such index.
	 *
	 * @param o element to search for
	...
	/**
	 * Retrieves and removes the last element of this list, or returns <tt>null</tt>
	 ...
	 * list-iterator's own <tt>remove</tt> or <tt>add</tt> methods, the
	 * list-iterator will throw a <tt>ConcurrentModificationException</tt>. Thus, in
	 * the face of concurrent modification, the iterator fails quickly and cleanly,
	 * rather than risking arbitrary, non-deterministic behavior at an undetermined
	 * time in the future.
	 *
