...
final class Order1 extends Curve {
	...
	public int compareTo(Curve other, double yrange[]) {
		...
		/*
		 * If "this" is curve A and "other" is curve B, then... xA(y) = x0A + (y - y0A)
		 * (x1A - x0A) / (y1A - y0A) xB(y) = x0B + (y - y0B) (x1B - x0B) / (y1B - y0B)
		 * xA(y) == xB(y) x0A + (y - y0A) (x1A - x0A) / (y1A - y0A) == x0B + (y - y0B)
		 * (x1B - x0B) / (y1B - y0B) 0 == x0A (y1A - y0A) (y1B - y0B) + (y - y0A) (x1A -
		 * x0A) (y1B - y0B) - x0B (y1A - y0A) (y1B - y0B) - (y - y0B) (x1B - x0B) (y1A -
		 * y0A) 0 == (x0A - x0B) (y1A - y0A) (y1B - y0B) + (y - y0A) (x1A - x0A) (y1B -
		 * y0B) - (y - y0B) (x1B - x0B) (y1A - y0A) If (dxA == x1A - x0A), etc... 0 ==
		 * (x0A - x0B) * dyA * dyB + (y - y0A) * dxA * dyB - (y - y0B) * dxB * dyA 0 ==
		 * (x0A - x0B) * dyA * dyB + y * dxA * dyB - y0A * dxA * dyB - y * dxB * dyA +
		 * y0B * dxB * dyA 0 == (x0A - x0B) * dyA * dyB + y * dxA * dyB - y * dxB * dyA
		 * - y0A * dxA * dyB + y0B * dxB * dyA 0 == (x0A - x0B) * dyA * dyB + y * (dxA *
		 * dyB - dxB * dyA) - y0A * dxA * dyB + y0B * dxB * dyA y == ((x0A - x0B) * dyA
		 * * dyB - y0A * dxA * dyB + y0B * dxB * dyA) / (-(dxA * dyB - dxB * dyA)) y ==
		 * ((x0A - x0B) * dyA * dyB - y0A * dxA * dyB + y0B * dxB * dyA) / (dxB * dyA -
		 * dxA * dyB)
		 */
		double dxa = x1 - x0;
		double dya = y1 - y0;
