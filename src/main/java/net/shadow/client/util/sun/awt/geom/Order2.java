...
package net.shadow.client.util.sun.awt.geom;
...
final class Order2 extends Curve {
	 ...
	 * quadratic Bezier curve. Put the parameters for the horizontal sections into
	 * the specified <code>ret</code> array. <p> If we examine the parametric
	 * equation in t, we have: Py(t) = C0*(1-t)^2 + 2*CP*t*(1-t) + C1*t^2 = C0 -
	 * 2*C0*t + C0*t^2 + 2*CP*t - 2*CP*t^2 + C1*t^2 = C0 + (2*CP - 2*C0)*t + (C0 -
	 * 2*CP + C1)*t^2 Py(t) = (C0 - 2*CP + C1)*t^2 + (2*CP - 2*C0)*t + (C0) If we
	 * take the derivative, we get: Py(t) = At^2 + Bt + C dPy(t) = 2At + B = 0 2*(C0
	 * - 2*CP + C1)t + 2*(CP - C0) = 0 2*(C0 - 2*CP + C1)t = 2*(C0 - CP) t = 2*(C0 -
	 * CP) / 2*(C0 - 2*CP + C1) t = (C0 - CP) / (C0 - CP + C1 - CP) Note that this
	 * method will return 0 if the equation is a line, which is either always
	 * horizontal or never horizontal. Completely horizontal curves need to be
	 * eliminated by other means outside of this method.
	 */
	...
	public static double TforY(double y, double ycoeff0, double ycoeff1, double ycoeff2) {
		 ...
		 * remember that these curves are constructed to be monotonic in Y and totally
		 * horizontal curves have already been eliminated. Now keep in mind that the Y
		 * coefficients of the polynomial form of the curve are calculated from the Y
		 * coordinates which define our curve. They should theoretically define the same
		 * curve, but they can be off by a couple of bits of precision after the math is
		 * done and so can represent a slightly modified curve. This is normally not an
		 * issue except when we have solutions near the endpoints. Since the answers we
		 * get from solving the polynomial may be off by a few bits that means that they
		 * could lie just a few bits of precision outside the [0,1] range.
		 *
		 * Another problem could be that while the parametric curve defined by the Y
		 * coordinates has a local minima or maxima at or just outside of the endpoints,
		 * the polynomial form might express that same min/max just inside of and just
		 * shy of the Y coordinate of that endpoint. In that case, if we solve for a Y
		 * coordinate at or near that endpoint, we may be solving for a Y coordinate
		 * that is below that minima or above that maxima and we would find no solutions
		 * at all.
		 *
		 * In either case, we can assume that y is so near one of the endpoints that we
		 * can just collapse it onto the nearest endpoint without losing more than a
		 * couple of bits of precision.
		 */
