...
import net.shadow.client.util.awt.geom.QuadCurve2D;
...
final class Order3 extends Curve {
	...
	/*
	 * Return the count of the number of horizontal sections of the specified cubic
	 * Bezier curve. Put the parameters for the horizontal sections into the
	 * specified <code>ret</code> array. <p> If we examine the parametric equation
	 * in t, we have: Py(t) = C0(1-t)^3 + 3CP0 t(1-t)^2 + 3CP1 t^2(1-t) + C1 t^3 =
	 * C0 - 3C0t + 3C0t^2 - C0t^3 + 3CP0t - 6CP0t^2 + 3CP0t^3 + 3CP1t^2 - 3CP1t^3 +
	 * C1t^3 Py(t) = (C1 - 3CP1 + 3CP0 - C0) t^3 + (3C0 - 6CP0 + 3CP1) t^2 + (3CP0 -
	 * 3C0) t + (C0) If we take the derivative, we get: Py(t) = Dt^3 + At^2 + Bt + C
	 * dPy(t) = 3Dt^2 + 2At + B = 0 0 = 3*(C1 - 3*CP1 + 3*CP0 - C0)t^2 + 2*(3*CP1 -
	 * 6*CP0 + 3*C0)t + (3*CP0 - 3*C0) 0 = 3*(C1 - 3*CP1 + 3*CP0 - C0)t^2 + 3*2*(CP1
	 * - 2*CP0 + C0)t + 3*(CP0 - C0) 0 = (C1 - CP1 - CP1 - CP1 + CP0 + CP0 + CP0 -
	 * C0)t^2 + 2*(CP1 - CP0 - CP0 + C0)t + (CP0 - C0) 0 = (C1 - CP1 + CP0 - CP1 +
	 * CP0 - CP1 + CP0 - C0)t^2 + 2*(CP1 - CP0 - CP0 + C0)t + (CP0 - C0) 0 = ((C1 -
	 * CP1) - (CP1 - CP0) - (CP1 - CP0) + (CP0 - C0))t^2 + 2*((CP1 - CP0) - (CP0 -
	 * C0))t + (CP0 - C0) Note that this method will return 0 if the equation is a
	 * line, which is either always horizontal or never horizontal. Completely
	 * horizontal curves need to be eliminated by other means outside of this
	 * method.
	 */
	...
	/*
	 * Solve the cubic whose coefficients are in the a,b,c,d fields and return the
	 * first root in the range [0, 1]. The cubic solved is represented by the
	 * equation: x^3 + (ycoeff2)x^2 + (ycoeff1)x + (ycoeff0) = y
	 * 
	...
	public Curve getSubCurve(double ystart, double yend, int dir) {
		...
		if (t0 > t1) {
			/*
			 * This happens in only rare cases where ystart is very near yend and solving
			 * for the yend root ends up stepping slightly lower in t than solving for the
			 * ystart root. Ideally we might want to skip this tiny little segment and just
			 * fudge the surrounding coordinates to bridge the gap left behind, but there is
			 * no way to do that from here. Higher levels could potentially eliminate these
			 * tiny "fixup" segments, but not without a lot of extra work on the code that
			 * coalesces chains of curves into subpaths. The simplest solution for now is to
			 * just reorder the t values and chop out a miniscule curve piece.
			 */
			double t = t0;
			t0 = t1;
			t1 = t;
