...
public abstract class AreaOp {
	...
	public static void resolveLinks(Vector subcurves, Vector chains, Vector links) {
		...
		while (chain != null || link != null) {
			/*
			 * Strategy 1: Connect chains or links if they are the only things left...
			 */
			...
			if (!connectchains && !connectlinks) {
				...
				/*
				 * Strategy 2: Connect chains or links if they close off an open area...
				 */
				...
				if (!connectchains && !connectlinks) {
					/*
					 * Strategy 3: Connect chains or links if their successor is between them and
					 * their potential connectee...
					 */
	...
	/*
	 * Does the position of the next edge at v1 "obstruct" the connectivity between
	 * current edge and the potential partner edge which is positioned at v2?
	 *
	 ...
	 * "entering into" type edges Allow 8-connected continuity for "exiting from"
	 * type edges
	 */
