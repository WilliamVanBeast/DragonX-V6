...
public interface RandomAccessRead extends Closeable {
	...
	/**
	 * Creates a random access read view starting at the given position with the
	 * given length.
	 * 
	 ...
	 * @throws IOException if something went wrong when creating the view for the
	 *                     RandomAccessRead
	 * 
	 */
	...
	/**
	 * Same as {@link #read(byte[])} but will loop until exactly length bytes are
	 * read or it will throw an exception.
	 *
	 * @param b The buffer to write the data to.
	 * @throws IOException
	 */
	...
	/**
	 * Same as {@link #read(byte[], int, int)} but will loop until exactly length
	 * bytes are read or it will throw an exception.
	 *
	 * @param b      The buffer to write the data to.
	 * @param offset Offset into the buffer to start writing.
