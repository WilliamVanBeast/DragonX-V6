...
public class RandomAccessReadBuffer implements RandomAccessRead {
	...
	private final Map<Long, RandomAccessReadBuffer> rarbCopies = new HashMap<>();
	...
	protected RandomAccessReadBuffer(int definedChunkSize) {
		...
		currentBuffer = EagRuntime.allocateByteBuffer(chunkSize);
		bufferList = new ObjectArrayList<>(1);
	...
	public RandomAccessReadBuffer(byte[] input) {
		...
		this(EagRuntime.allocateByteBuffer(input.length).put(input).flip());
	}
	...
	/**
	 * Create a random access read buffer of the given input stream by copying the
	 * data to the memory.
	 * 
	...
	public RandomAccessReadBuffer(InputStream input) throws IOException {
		...
		while (remainingBytes > 0 && (bytesRead = input.read(currentBuffer.array(), offset, remainingBytes)) > -1) {
	...
	private RandomAccessReadBuffer(RandomAccessReadBuffer parent) {
		...
		bufferList = new ObjectArrayList<>(parent.bufferList.size());
	...
	/**
	 * Create e new RandomAccessReadBuffer using the given InputStream. The data is
	 * copied to the memory and the InputStream is closed.
	 * 
