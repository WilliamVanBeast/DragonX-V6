 ...
 *      context-free, and the execution of the program can modify the the
...
class Type1Lexer {
	 ...
	 * Log instance.
	 */
	private static final Logger LOG = LogManager.getLogger("Type1Lexer");
	...
	Type1Lexer(byte[] bytes) throws IOException {
		buffer = EagRuntime.allocateByteBuffer(bytes.length).put(bytes).flip();
	...
	/**
	 * Checks if the kind of the next token equals the given one without consuming
	 * it.
	 * 
			...
			while (buffer.hasRemaining()) {
				...
				if (c == '%') {
					...
					if (regular == null) {
						...
						throw new DamagedFontException("Could not read token at position " + buffer.position());
					}
