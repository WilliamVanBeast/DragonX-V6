...
public class CMap {
	...
	// code lengths
	private final List<CodespaceRange> codespaceRanges = new ObjectArrayList<>();

	// Unicode mappings
	// one byte input values
	private final Int2ObjectMap<String> charToUnicodeOneByte = new Int2ObjectOpenHashMap<>();
	// two byte input values
	private final Int2ObjectMap<String> charToUnicodeTwoBytes = new Int2ObjectOpenHashMap<>();
	// 3 / 4 byte input values
	private final Int2ObjectMap<String> charToUnicodeMoreBytes = new Int2ObjectOpenHashMap<>();

	// CID mappings
	// map with all code to cid mappings organized by the origin byte length of the
	// input value
	private final Int2ObjectMap<Int2IntMap> codeToCid = new Int2ObjectOpenHashMap<>();
	private final List<CIDRange> codeToCidRanges = new ObjectArrayList<>();

	// inverted map
	private final Object2ObjectMap<String, byte[]> unicodeToByteCodes = new Object2ObjectOpenHashMap<>();
	 ...
	 * values aren't unique. <br>
	 * Example:<br>
	 * The two byte value 0x00, 0x65 maps to 0x20 <br>
	 * An input value of 0x65 always returns 0x20 even if the value has an origin
	 * byte length of 1.
	 *
	...
	public int readCode(InputStream in) throws IOException {
		...
		for (int i = 0; i < maxCodeLength; ++i) {
			sb.append(HString.format("0x%02X (%04o) ", bytes[i], bytes[i]));
		}
		...
		if (in.markSupported()) {
			...
			LOG.warn("mark() and reset() not supported, {} bytes have been skipped", maxCodeLength - 1);
		}
	...
	void addCIDMapping(byte[] code, int cid) {
		Int2IntMap codeToCidMap = codeToCid.get(code.length);
		if (codeToCidMap == null) {
			codeToCidMap = new Int2IntOpenHashMap();
	...
	/**
	 * Implementation of the usecmap operator. This will copy all of the mappings
	 * from one cmap to another.
	 * 
	...
	void useCmap(CMap cmap) {
		...
		cmap.charToUnicodeTwoBytes.forEach(
				(k, v) -> unicodeToByteCodes.put(v, new byte[] { (byte) ((k >>> 8) & 0xFF), (byte) (k & 0xFF) }));
		cmap.charToUnicodeMoreBytes.forEach((k, v) -> {
			byte[] bar;
			if (k <= 0xFFFFFF) {
				// 3 bytes
				bar = new byte[] { (byte) ((k >>> 16) & 0xFF), (byte) ((k >>> 8) & 0xFF), (byte) (k & 0xFF) };
			} else {
				// 4 bytes
				bar = new byte[] { (byte) ((k >>> 24) & 0xFF), (byte) ((k >>> 16) & 0xFF), (byte) ((k >>> 8) & 0xFF),
						(byte) (k & 0xFF) };
			}
