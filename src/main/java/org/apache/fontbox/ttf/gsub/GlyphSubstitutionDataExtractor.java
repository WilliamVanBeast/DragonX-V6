...
/**
 * This class has utility methods to extract meaningful GsubData from the highly
 * obfuscated GSUB Tables. This GsubData is then used to determine which
 * combination of glyphs or words have to be replaced.
 *
 * @author Palash Ray
 * 
 */
public class GlyphSubstitutionDataExtractor {
	private static final Logger LOG = LogManager.getLogger("GlyphSubstitutionDataExtractor");

	public GsubData getGsubData(Map<String, ScriptTable> scriptList, FeatureListTable featureListTable,
	...
	/**
	 * Unlike {@link #getGsubData(Map, FeatureListTable, LookupListTable)}, this
	 * method doesn't iterate over supported {@link Language}'s searching for the
	 * first match with the scripts of the font. Instead, it unconditionally creates
	 * {@link ScriptTableDetails} instance with language left
	 * {@linkplain Language#UNSPECIFIED unspecified}.
	 * 
	 * @param scriptName
	 * @param scriptTable
	 * @param featureListTable
	 * @param lookupListTable
	 * @return {@link GsubData} instance built especially for the given
	 *         {@code scriptName}
	 */
	public GsubData getGsubData(String scriptName, ScriptTable scriptTable, FeatureListTable featureListTable,
			LookupListTable lookupListTable) {
	...
	private MapBackedGsubData buildMapBackedGsubData(FeatureListTable featureListTable, LookupListTable lookupListTable,
			ScriptTableDetails scriptTableDetails) {
		...
		if (scriptTable.getDefaultLangSysTable() != null) {
			populateGsubData(gsubData, scriptTable.getDefaultLangSysTable(), featureListTable, lookupListTable);
		}
	...
	private void populateGsubData(Map<String, Map<List<Integer>, List<Integer>>> gsubData, LangSysTable langSysTable,
	...
	private void populateGsubData(Map<String, Map<List<Integer>, List<Integer>>> gsubData, FeatureRecord featureRecord,
			LookupListTable lookupListTable) {
	...
	private void extractData(Map<List<Integer>, List<Integer>> glyphSubstitutionMap, LookupTable lookupTable) {
	...
	private void extractDataFromSingleSubstTableFormat1Table(Map<List<Integer>, List<Integer>> glyphSubstitutionMap,
	...
	private void extractDataFromSingleSubstTableFormat2Table(Map<List<Integer>, List<Integer>> glyphSubstitutionMap,
	...
	private void extractDataFromMultipleSubstitutionFormat1Table(Map<List<Integer>, List<Integer>> glyphSubstitutionMap,
		...
		if (coverageTable.getSize() != multipleSubstFormat1Subtable.getSequenceTables().length) {
			LOG.warn("The coverage table size ({}) should be the same as the count of the sequence tables ({})",
		...
		for (int i = 0; i < coverageTable.getSize(); i++) {
			...
			List<Integer> substituteGlyphIDList = new ObjectArrayList<>(substituteGlyphIDArray.length);
	...
	/**
	 * Extracts data from the AlternateSubstitutionFormat1 (lookuptype) 3 table and
	 * puts it in the glyphSubstitutionMap.
	 *
	...
	private void extractDataFromAlternateSubstitutionSubstFormat1Table(
		...
		for (int i = 0; i < coverageTable.getSize(); i++) {
			...
			// Loop through the substitute glyphs and pick the first one that is not the
			// same as the coverage glyph
	...
	private void extractDataFromLigatureTable(Map<List<Integer>, List<Integer>> glyphSubstitutionMap,
		...
		List<Integer> glyphsToBeSubstituted = new ObjectArrayList<>(componentGlyphIDs.length);
	...
	private void putNewSubstitutionEntry(Map<List<Integer>, List<Integer>> glyphSubstitutionMap,
		...
		if (oldValues != null) {
			LOG.debug("For the newGlyph: {}, newValue: {} is trying to override the oldValue {}", newGlyphList,
					glyphsToBeSubstituted, oldValues);
		}
	}
