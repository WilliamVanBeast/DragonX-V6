...
public class GsubWorkerForDevanagari implements GsubWorker {
	private static final Logger LOG = LogManager.getLogger("GsubWorkerForDevanagari");
	 ...
	 */
	private static final List<String> FEATURES_IN_ORDER = Arrays.asList("locl", "nukt", "akhn", "rphf", RKRF_FEATURE,
			"blwf", "half", VATU_FEATURE, "cjct", "pres", "abvs", "blws", "psts", "haln", "calt");
		...
		for (String feature : FEATURES_IN_ORDER) {
			if (!gsubData.isFeatureSupported(feature)) {
				if (feature.equals(RKRF_FEATURE) && gsubData.isFeatureSupported(VATU_FEATURE)) {
					...
					intermediateGlyphsFromGsub = applyRKRFFeature(gsubData.getFeature(VATU_FEATURE),
	...
	private List<Integer> applyRKRFFeature(ScriptFeature rkrfGlyphsForSubstitution, List<Integer> originalGlyphIds) {
		...
		List<Integer> rkrfList = new ObjectArrayList<>(originalGlyphIds);
	...
	private List<Integer> adjustRephPosition(List<Integer> originalGlyphIds) {
		List<Integer> rephAdjustedList = new ObjectArrayList<>(originalGlyphIds);
	...
	private List<Integer> getRephGlyphIds() {
		List<Integer> result = new ObjectArrayList<>(REPH_CHARS.length);
	...
	private List<Integer> getBeforeRephGlyphIds() {
		List<Integer> glyphIds = new ObjectArrayList<>(BEFORE_REPH_CHARS.length);
