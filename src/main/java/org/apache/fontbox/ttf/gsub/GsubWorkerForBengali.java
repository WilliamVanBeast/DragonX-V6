...
public class GsubWorkerForBengali implements GsubWorker {
	private static final Logger LOG = LogManager.getLogger("GsubWorkerForBengali");
	...
	private List<Integer> repositionBeforeHalfGlyphIds(List<Integer> originalGlyphIds) {
		List<Integer> repositionedGlyphIds = new ObjectArrayList<>(originalGlyphIds);
	...
	private List<Integer> repositionBeforeAndAfterSpanGlyphIds(List<Integer> originalGlyphIds) {
		...
		for (int index = 1; index < originalGlyphIds.size(); index++) {
			int glyphId = originalGlyphIds.get(index);
			BeforeAndAfterSpanComponent beforeAndAfterSpanComponent = beforeAndAfterSpanGlyphIds.get(glyphId);
	...
	private List<Integer> applyGsubFeature(ScriptFeature scriptFeature, List<Integer> originalGlyphs) {
		...
		List<Integer> gsubProcessedGlyphs = new ObjectArrayList<>(tokens.size());
	...
	private List<Integer> getBeforeHalfGlyphIds() {
		List<Integer> glyphIds = new ObjectArrayList<>(BEFORE_HALF_CHARS.length);
	...
	private Map<Integer, BeforeAndAfterSpanComponent> getBeforeAndAfterSpanGlyphIds() {
		Map<Integer, BeforeAndAfterSpanComponent> result = new Int2ObjectOpenHashMap<>();
	...
	/**
	 * Models characters like O-kar (\u09CB) and OU-kar (\u09CC). Since these 2
	 * characters is represented by 2 components, one before and one after the
	 * Vyanjan Varna on which this is used, this glyph has to be replaced by these 2
	 * glyphs. For O-kar, it has to be replaced by E-kar (\u09C7) and AA-kar
	 * (\u09BE). For OU-kar, it has be replaced by E-kar (\u09C7) and \u09D7.
	 *
	 */
