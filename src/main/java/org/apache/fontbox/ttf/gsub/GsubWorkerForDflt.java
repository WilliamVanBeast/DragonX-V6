 ...
 *
 * <p>
 * According to the OpenType specification, a Script table with the script tag
 * 'DFLT' (default) is used in fonts to define features that are not
 * script-specific. Applications should use the DFLT script table when no script
 * table exists for the specific script of the text being processed, or when
 * text lacks a defined script (containing only symbols or punctuation).
 * </p>
 *
 * <p>
 * This implementation applies common, script-neutral typographic features that
 * work across writing systems. The feature order follows standard OpenType
 * recommendations for universal glyph substitutions.
 * </p>
 *
 * <p>
 * Reference: <a href=
 * "https://learn.microsoft.com/en-us/typography/opentype/spec/chapter2#scriptlist-table">
 * OpenType ScriptList Table Specification</a>
 * </p>
 */
public class GsubWorkerForDflt implements GsubWorker {
	private static final Logger LOG = LogManager.getLogger("GsubWorkerForDflt");
	 ...
	 * </ul>
	 *
	 * Note: This feature list focuses on common GSUB (substitution) features. GPOS
	 * features like 'kern', 'mark', 'mkmk' are handled separately.
	 */
	...
	private List<Integer> applyGsubFeature(ScriptFeature scriptFeature, List<Integer> originalGlyphs) {
		...
		List<Integer> gsubProcessedGlyphs = new ObjectArrayList<>();
