...
/**
 * An implementation of the TTFDataStream using RandomAccessRead as source. The
 * underlying RandomAccessRead can be any length, but this implementation
 * supports only bufferLengths upto Integer.MAX_VALUE.
 */
class RandomAccessReadDataStream extends TTFDataStream {
	...
	RandomAccessReadDataStream(RandomAccessRead randomAccessRead) throws IOException {
		...
		int amountRead;
		while ((amountRead = randomAccessRead.read(data, data.length - remainingBytes, remainingBytes)) > 0) {
	...
	/**
	 * Seek into the datasource. When the requested <code>pos</code> is &lt; 0, an
	 * IOException() is fired. When the requested <code>pos</code> is &ge;
	 * {@link #length}, the {@link #currentPosition} is set to the first byte
	 * <i>after</i> the {@link #data}!
	 *
