...
public class TTFParser {
	private static final Logger LOG = LogManager.getLogger("TTFParser");
	 ...
	 *
	 * @param randomAccessRead The RandomAccessREad to be read from. It will be
	 ...
	 *
	 * @param inputStream The TTF data stream to parse from. It will be closed
	 *                    before returning.
	 * @return A TrueType font.
	 * @throws IOException If there is an error parsing the TrueType font.
	 */
		...
		for (int i = 0; i < numberOfTables; i++) {
			...
			if (table != null) {
				if (table.getOffset() + table.getLength() > font.getOriginalDataSize()) {
					...
					LOG.warn("Skip table '{}' which goes past the file size; offset: {}, size: {}, font size: {}",
	...
	/**
	 * Based on {@link #parseTables()}. Parse all table headers and check if all
	...
	FontHeaders parseTableHeaders(TTFDataStream raf) throws IOException {
		...
		try (TrueTypeFont font = createFontWithTables(raf)) {
			...
			final String[] mandatoryTables = { HeaderTable.TAG, HorizontalHeaderTable.TAG, MaximumProfileTable.TAG,
					isEmbedded ? null : PostScriptTable.TAG, // in an embedded font this table is optional
					isOTFAndPostScript ? null : IndexToLocationTable.TAG, isOTFAndPostScript ? null : GlyphTable.TAG,
					isEmbedded ? null : NamingTable.TAG, HorizontalMetricsTable.TAG,
