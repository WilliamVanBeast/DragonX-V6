 ...
 *
 * @see <a href=
 *      "http://xmlgraphics.apache.org/batik">http://xmlgraphics.apache.org/batik</a>
 * 
 *      Contour rendering ported from PDF.js, viewed on 14.2.2015, rev 2e97c0d
 *
 * @see <a href=
 *      "https://github.com/mozilla/pdf.js/blob/c0d17013a28ee7aa048831560b6494a26c52360c/src/core/font_renderer.js">pdf.js/src/core/font_renderer.js</a>
 *
 */
class GlyphRenderer {
	private static final Logger LOG = LogManager.getLogger("GlyphRenderer");
	...
	private GeneralPath calculatePath(Point[] points) {
		...
		for (int p = 0, len = points.length; p < len; ++p) {
			if (points[p].endOfContour) {
				...
				List<Point> contour = new ObjectArrayList<>((p - start) + 3);
	...
	private void moveTo(GeneralPath path, Point point) {
		...
		LOG.trace("moveTo: {}", HString.format("%d,%d", point.x, point.y));
	}

	private void lineTo(GeneralPath path, Point point) {
		path.lineTo(point.x, point.y);
		LOG.trace("lineTo: {}", HString.format("%d,%d", point.x, point.y));
	}

	private void quadTo(GeneralPath path, Point ctrlPoint, Point point) {
		path.quadTo(ctrlPoint.x, ctrlPoint.y, point.x, point.y);
		LOG.trace("quadTo: {}", HString.format("%d,%d %d,%d", ctrlPoint.x, ctrlPoint.y, point.x, point.y));
	}
	...
	private static class Point {
		...
		public String toString() {
			return HString.format("Point(%d,%d,%s,%s)", x, y, onCurve ? "onCurve" : "",
