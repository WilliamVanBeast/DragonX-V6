...
public class CFFCIDFont extends CFFFont {
	...
	private final Int2ObjectMap<CIDKeyedType2CharString> charStringCache = new Int2ObjectOpenHashMap<>();
	...
	public CIDKeyedType2CharString getType2CharString(int cid) throws IOException {
		...
		if (type2 == null) {
			...
			List<Object> type2seq = getParser().parse(bytes, globalSubrIndex, getLocalSubrIndex(gid));
			type2 = new CIDKeyedType2CharString(reader, getName(), cid, gid, type2seq, getDefaultWidthX(gid),
	...
	/**
	 * Private implementation of Type1CharStringReader, because only CFFType1Font
	 * can expose this publicly, as CIDFonts only support this for legacy 'seac'
	 * commands.
	 */
