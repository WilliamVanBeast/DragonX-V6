...
import net.shadow.client.util.awt.geom.GeneralPath;
...
public class Type1CharString {
	private static final Logger LOG = LogManager.getLogger("Type1CharString");
	...
	private final List<Point2D.Float> flexPoints = new ObjectArrayList<>();
	private final List<Object> type1Sequence = new ObjectArrayList<>();
	...
	private void render() {
		...
		width = 0;
		List<Number> numbers = new ObjectArrayList<>();
	...
	private void handleType1Command(List<Number> numbers, CharStringCommand command) {
		...
		case RRCURVETO:
			if (numbers.size() >= 6) {
				rrcurveTo(numbers.get(0), numbers.get(1), numbers.get(2), numbers.get(3), numbers.get(4),
						numbers.get(5));
			}
			break;
		case CLOSEPATH:
		...
		case CALLSUBR:
			// indicates an invalid charstring
			LOG.warn("Unexpected charstring command: {} in glyph {} of font {}", type1KeyWord, glyphName, fontName);
			break;
		default:
	...
	/**
	 * Sets the current absolute point without performing a moveto. Used only with
	 * results from callothersubr
	 */
	...
	private void callOtherSubr(int num) {
		if (num == 0) {
			...
			if (flexPoints.size() < 7) {
				LOG.warn("flex without moveTo in font {}, glyph {}, command {}", fontName, glyphName, commandCount);
				return;
			}
	...
	private void seac(Number asb, Number adx, Number ady, Number bchar, Number achar) {
		...
		try {
			...
			LOG.warn("invalid seac character in glyph {} of font {}", glyphName, fontName);
			LOG.warn(e);
		}
		// accent character
