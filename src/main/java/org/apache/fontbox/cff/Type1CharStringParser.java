...
public class Type1CharStringParser {
	private static final Logger LOG = LogManager.getLogger("Type1CharStringParser");
	...
	public List<Object> parse(byte[] bytes, List<byte[]> subrs, String glyphName) throws IOException {
		...
		return parse(bytes, subrs, new ObjectArrayList<>());
	}
	...
	private void processCallSubr(List<byte[]> subrs, List<Object> sequence) throws IOException {
		...
		if (!(obj instanceof Integer)) {
			LOG.warn("Parameter {} for CALLSUBR is ignored, integer expected in glyph '{}' of font {}", obj,
					currentGlyph, fontName);
			return;
		}
		int operand = (int) obj;
		...
		if (operand >= 0 && operand < subrs.size()) {
			...
			LOG.warn("CALLSUBR is ignored, operand: {}, subrs.size(): {} in glyph '{}' of font {}", operand,
					subrs.size(), currentGlyph, fontName);
	...
	private void processCallOtherSubr(DataInput input, List<Object> sequence) throws IOException {
		...
		if (!results.isEmpty()) {
			LOG.warn("Value left on the PostScript stack in glyph {} of font {}", currentGlyph, fontName);
		}
	}

	// this method is a workaround for the fact that Type1CharStringParser assumes
	// that subrs and
	// othersubrs can be unrolled without executing the 'div' operator, which isn't
	// true
