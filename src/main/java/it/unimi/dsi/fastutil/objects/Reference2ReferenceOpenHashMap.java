...
public class Reference2ReferenceOpenHashMap<K, V> extends AbstractReference2ReferenceMap<K, V>
	...
	public V put(final K k, final V v) {
		// The starting point.
		int pos = ((k) == null ? 0x87fcd5c : it.unimi.dsi.fastutil.HashCommon.murmurHash3(System.identityHashCode(k)))
				& mask;
		...
		for (;;) {
			...
			while (used[pos]) {
				slot = ((key[pos]) == null ? 0x87fcd5c
						: it.unimi.dsi.fastutil.HashCommon.murmurHash3(System.identityHashCode(key[pos]))) & mask;
	...
	private final class MapEntry implements Reference2ReferenceMap.Entry<K, V>, Map.Entry<K, V> {
		...
		public int hashCode() {
			return ((key[index]) == null ? 0 : System.identityHashCode(key[index]))
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			if (pos < 0) {
				...
				// The starting point.
				int pos = ((k) == null ? 0x87fcd5c
						: it.unimi.dsi.fastutil.HashCommon.murmurHash3(System.identityHashCode(k))) & mask;
				// There's always an unused entry.
				while (used[pos]) {
					if (((key[pos]) == (k)))
						return pos;
		...
		protected final int shiftKeys(int pos) {
			...
			for (;;) {
				...
				if (pos < last) {
					// Wrapped entry.
					if (wrapped == null)
						wrapped = new ReferenceArrayList<K>();
					wrapped.add(key[pos]);
				}
				key[last] = key[pos];
				value[last] = value[pos];
	...
	protected void rehash(final int newN) {
		...
		for (int j = size; j-- != 0;) {
			while (!used[i])
				i++;
			k = key[i];
			pos = ((k) == null ? 0x87fcd5c : it.unimi.dsi.fastutil.HashCommon.murmurHash3(System.identityHashCode(k)))
					& newMask;
			while (newUsed[pos])
				pos = (pos + 1) & newMask;
			newUsed[pos] = true;
