...
public class SkeletonEsp extends Module {
    ...
    private FloatSetting width = new FloatSetting("width",1.0f, 0.5f, 10.0f);
    private BooleanSetting visibleonly = new BooleanSetting("visibleonly", false);

    public SkeletonEsp() {
        super("SkeletonEsp", Category.RENDER);
        addSettings(width, visibleonly);
    ...
    public void onRender3DEvent(Render3DEvent e) {
        ...
        mc.world.playerEntities.forEach(player -> drawSkeleton(e, player));
    ...
    private void drawSkeleton(Render3DEvent event, EntityPlayer e) {
        if (!e.isInvisible()) {
            ...
            if (entPos != null && e.getEntityId() != -1488 && e.isEntityAlive() && !e.isDead && e != mc.player && !e.isPlayerSleeping() && (!visibleonly.get() || mc.player.canEntityBeSeen(e))) {
                GL11.glPushMatrix();
                GL11.glEnable(GL11.GL_LINE_SMOOTH);
                GL11.glLineWidth(width.get());
                GlStateManager.color(1,1,1, 1);
                Vec3d vec = getVec3(event, e);
                double x = vec.xCoord - Minecraft.getMinecraft().getRenderManager().renderPosX;
                double y = vec.yCoord - Minecraft.getMinecraft().getRenderManager().renderPosY;
                double z = vec.zCoord - Minecraft.getMinecraft().getRenderManager().renderPosZ;
                GL11.glTranslated(x, y, z);
                float xOff = e.prevRenderYawOffset + (e.renderYawOffset - e.prevRenderYawOffset) * mc.timer.field_194148_c;
    ...
    private Vec3d getVec3(Render3DEvent event, EntityPlayer var0) {
        float pt = mc.timer.field_194148_c;
