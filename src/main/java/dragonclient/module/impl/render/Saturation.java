...
import dragonclient.util.saturation.FoodValues;
import dragonclient.util.saturation.SaturationHelper;
...
public class Saturation extends Module {
        ...
        private final FloatSetting maxFlashAlpha = new FloatSetting("Max Flash Alpha", 0.65f, 0f, 1f);
    ...
    public Saturation() {
        ...
        final ItemStack heldItem = mc.player.getHeldItem(EnumHand.MAIN_HAND);
        ...
        final FoodValues foodValues = SaturationHelper.getFoodValues(heldItem);
        ...
        this.drawHungerOverlay(foodHunger, stats.getFoodLevel(), right, top, flashAlpha, SaturationHelper.isRottenFood(heldItem));
        ...
        flashAlpha = Math.max(0.0F, Math.min(1.0F, unclampedFlashAlpha)) * Math.min(1.0F, maxFlashAlpha.get());
    }
    ...
    private void generateHungerBarOffsets(final int right, final int top, final int ticks) {
        ...
        for (int i = startSaturationBar; i < endSaturationBar; ++i) {
            ...
            for (final PotionEffect e : mc.player.getActivePotionEffects()) {
                if (e.getPotion() == MobEffects.HUNGER) {
                    u += 4 * iconSize;
                    break;
                }
            }
